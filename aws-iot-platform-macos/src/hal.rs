...
impl PlatformHAL for MacOSHAL {
    async fn sleep(&self, duration: Duration) -> PlatformResult<()> {
        println!("{}{}💤 SLEEP:{} Sleeping for {:?}{}", 
                colors::BOLD, colors::BLUE, colors::RESET, duration, colors::RESET);
        
        let start_time = std::time::Instant::now();
        tokio::time::sleep(duration).await;
        let actual_duration = start_time.elapsed();
        
        println!("{}{}⏰ WAKE:{} Sleep completed (actual: {:?}){}", 
                colors::BOLD, colors::GREEN, colors::RESET, actual_duration, colors::RESET);
        
        tracing::info!("Sleep operation completed: requested={:?}, actual={:?}", duration, actual_duration);
        Ok(())
    }
        ...
        match state {
            LedState::On => {
                println!("{}{}💡 LED:{} {} ON {} (was: {}){}", 
                        ...
                        if previous_state == LedState::On { "ON" } else { "OFF" },
                        colors::RESET);
            },
            LedState::Off => {
                println!("{}{}🔌 LED:{} {} OFF {} (was: {}){}", 
    ...
    async fn store_secure_data(&self, key: &str, data: &[u8]) -> PlatformResult<()> {
        println!("{}{}🔐 STORAGE:{} Storing secure data for key: '{}' ({} bytes){}", 
                colors::BOLD, colors::MAGENTA, colors::RESET, key, data.len(), colors::RESET);
        ...
        println!("{}{}✅ KEYCHAIN:{} Secure data stored successfully for key: '{}'{}", 
                colors::BOLD, colors::GREEN, colors::RESET, key, colors::RESET);
    ...
    async fn load_secure_data(&self, key: &str) -> PlatformResult<Option<Vec<u8>>> {
        println!("{}{}🔓 STORAGE:{} Loading secure data for key: '{}'{}", 
                colors::BOLD, colors::CYAN, colors::RESET, key, colors::RESET);
        ...
        match tokio::fs::read(&file_path).await {
            Ok(data) => {
                println!("{}{}✅ STORAGE:{} Secure data loaded successfully for key: '{}' ({} bytes){}", 
                        colors::BOLD, colors::GREEN, colors::RESET, key, data.len(), colors::RESET);
                tracing::debug!("Secure data loaded successfully for key: {}", key);
                Ok(Some(data))
            },
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                println!("{}{}❌ STORAGE:{} No secure data found for key: '{}'{}", 
                        colors::BOLD, colors::RED, colors::RESET, key, colors::RESET);
    ...
    async fn delete_secure_data(&self, key: &str) -> PlatformResult<bool> {
        println!("{}{}🗑️ STORAGE:{} Deleting secure data for key: '{}'{}", 
                colors::BOLD, colors::RED, colors::RESET, key, colors::RESET);
        ...
        match tokio::fs::remove_file(&file_path).await {
            Ok(()) => {
                println!("{}{}✅ STORAGE:{} Secure data deleted successfully for key: '{}'{}", 
                        colors::BOLD, colors::GREEN, colors::RESET, key, colors::RESET);
                tracing::debug!("Secure data deleted successfully for key: {}", key);
                Ok(true)
            },
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                println!("{}{}❌ STORAGE:{} No secure data found to delete for key: '{}'{}", 
                        colors::BOLD, colors::YELLOW, colors::RESET, key, colors::RESET);
    ...
    async fn initialize(&mut self) -> PlatformResult<()> {
        ...
        println!("{}{}🚀 INIT:{} Initializing macOS HAL...{}", 
                colors::BOLD, colors::BLUE, colors::RESET, colors::RESET);
        ...
        println!("{}{}🔐 STORAGE:{} Secure storage initialized{}", 
                colors::BOLD, colors::GREEN, colors::RESET, colors::RESET);
        
        // Initialize LED state
        *self.led_state.lock().await = LedState::Off;
        
        *initialized = true;
        println!("{}{}✅ INIT:{} macOS HAL initialized successfully{}", 
                colors::BOLD, colors::GREEN, colors::RESET, colors::RESET);
    ...
    async fn shutdown(&mut self) -> PlatformResult<()> {
        ...
        println!("{}{}🛑 SHUTDOWN:{} Shutting down macOS HAL...{}", 
                colors::BOLD, colors::RED, colors::RESET, colors::RESET);
        
        // Reset LED state
        *self.led_state.lock().await = LedState::Off;
        println!("{}{}🔌 LED:{} LED turned OFF during shutdown{}", 
                colors::BOLD, colors::YELLOW, colors::RESET, colors::RESET);
        
        *initialized = false;
        println!("{}{}✅ SHUTDOWN:{} macOS HAL shutdown successfully{}", 
                colors::BOLD, colors::GREEN, colors::RESET, colors::RESET);
...
mod tests {
    ...
    async fn test_sleep_functionality() {
        ...
        assert!(elapsed >= sleep_duration, "Sleep should take at least the requested duration");
        assert!(elapsed < sleep_duration + Duration::from_millis(50), "Sleep should not take much longer than requested");
    }
    ...
    async fn test_led_state_management() {
        ...
        let initial_state = hal.get_led_state().await.expect("Failed to get initial LED state");
        ...
        let current_state = hal.get_led_state().await.expect("Failed to get LED state after turning ON");
        ...
        let current_state = hal.get_led_state().await.expect("Failed to get LED state after turning OFF");
    ...
    async fn test_led_state_transitions() {
        ...
        for expected_state in transitions {
            ...
            assert_eq!(actual_state, expected_state, "LED state should match expected state");
        }
    }
    ...
    async fn test_device_info() {
        ...
        let device_info = hal.get_device_info().await.expect("Failed to get device info");
        
        // Verify device info structure
        assert!(!device_info.device_id.is_empty(), "Device ID should not be empty");
        assert!(device_info.platform.contains("macOS"), "Platform should contain 'macOS'");
        assert!(!device_info.version.is_empty(), "Version should not be empty");
        assert!(!device_info.firmware_version.is_empty(), "Firmware version should not be empty");
        assert!(device_info.hardware_revision.is_some(), "Hardware revision should be present");
        assert!(device_info.serial_number.is_some(), "Serial number should be present");
        
        // Verify device ID format
        assert!(device_info.device_id.starts_with("macos-"), "Device ID should start with 'macos-'");
    }
    ...
    async fn test_memory_info() {
        ...
        let memory_info = hal.get_memory_info().await.expect("Failed to get memory info");
        
        // Verify memory info structure
        assert!(memory_info.total_bytes > 0, "Total memory should be greater than 0");
        assert!(memory_info.free_bytes > 0, "Free memory should be greater than 0");
        assert!(memory_info.used_bytes > 0, "Used memory should be greater than 0");
        assert!(memory_info.largest_free_block > 0, "Largest free block should be greater than 0");
    ...
    async fn test_uptime_info() {
        ...
        assert!(uptime_info.uptime > Duration::ZERO, "Uptime should be greater than zero");
        assert!(uptime_info.boot_time <= Utc::now(), "Boot time should be in the past or now");
        
        // Verify uptime is reasonable (should be small for test)
        assert!(uptime_info.uptime < Duration::from_secs(60), "Uptime should be less than 60 seconds for test");
    }
    ...
    async fn test_secure_storage_basic_operations() {
        ...
        // Test loading data
        let loaded_data = hal.load_secure_data(test_key).await.expect("Failed to load secure data");
        assert!(loaded_data.is_some(), "Loaded data should exist");
        assert_eq!(loaded_data.unwrap(), test_data, "Loaded data should match stored data");
        
        // Test deleting data
        let deleted = hal.delete_secure_data(test_key).await.expect("Failed to delete secure data");
        assert!(deleted, "Data should be successfully deleted");
        
        // Test loading deleted data
        let loaded_data = hal.load_secure_data(test_key).await.expect("Failed to load secure data after deletion");
    ...
    async fn test_secure_storage_nonexistent_key() {
        ...
        let loaded_data = hal.load_secure_data(nonexistent_key).await.expect("Failed to load nonexistent secure data");
        assert!(loaded_data.is_none(), "Nonexistent data should return None");
        
        // Test deleting nonexistent data
        let deleted = hal.delete_secure_data(nonexistent_key).await.expect("Failed to delete nonexistent secure data");
    ...
    async fn test_secure_storage_overwrite() {
        ...
        assert!(result.is_ok(), "Storing original secure data should succeed");
        ...
        let loaded_data = hal.load_secure_data(test_key).await.expect("Failed to load overwritten secure data");
        assert!(loaded_data.is_some(), "Overwritten data should exist");
        assert_eq!(loaded_data.unwrap(), new_data, "Loaded data should match new data");
    ...
    async fn test_secure_storage_empty_data() {
        ...
        let loaded_data = hal.load_secure_data(test_key).await.expect("Failed to load empty secure data");
        assert!(loaded_data.is_some(), "Empty data should exist");
        assert_eq!(loaded_data.unwrap(), empty_data, "Loaded empty data should match");
    ...
    async fn test_secure_storage_large_data() {
        ...
        let loaded_data = hal.load_secure_data(test_key).await.expect("Failed to load large secure data");
        assert!(loaded_data.is_some(), "Large data should exist");
        assert_eq!(loaded_data.unwrap(), large_data, "Loaded large data should match");
    ...
    async fn test_secure_storage_special_characters() {
        ...
        assert!(result.is_ok(), "Storing secure data with special chars should succeed");
        
        // Test loading data with special characters
        let loaded_data = hal.load_secure_data(test_key).await.expect("Failed to load secure data with special chars");
        assert!(loaded_data.is_some(), "Data with special chars should exist");
        assert_eq!(loaded_data.unwrap(), special_data, "Loaded special char data should match");
    ...
    async fn test_list_secure_keys() {
        ...
        for key in &test_keys {
            ...
            assert!(result.is_ok(), "Storing secure data for listing should succeed");
        }
        
        // List keys
        let listed_keys = hal.list_secure_keys().await.expect("Failed to list secure keys");
        
        // Verify all test keys are present
        for key in &test_keys {
            assert!(listed_keys.contains(&key.to_string()), "Listed keys should contain test key: {}", key);
        }
    ...
    async fn test_shutdown_before_initialization() {
        ...
        let result = hal.shutdown().await;
        assert!(result.is_err(), "Shutdown without initialization should fail");
    }
    ...
    async fn test_concurrent_led_operations() {
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                let state = if i % 2 == 0 { LedState::On } else { LedState::Off };
        ...
        assert!(final_state.is_ok(), "Getting LED state after concurrent operations should succeed");
    }
    ...
    async fn test_concurrent_secure_storage_operations() {
        ...
        for handle in handles {
            let result = handle.await.expect("Task should complete");
            assert!(result.is_ok(), "Concurrent storage operation should succeed");
        }
    }
}