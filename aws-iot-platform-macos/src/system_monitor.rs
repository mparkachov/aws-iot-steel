use aws_iot_core::{DeviceInfo, MemoryInfo, PlatformError, PlatformResult, UptimeInfo};
...
impl MacOSSystemMonitor {
    ...
    pub async fn get_device_info(&self) -> PlatformResult<DeviceInfo> {
        ...
        let serial_number = self
            .get_system_serial()
            .await
            .unwrap_or_else(|| format!("SIM-{}", uuid::Uuid::new_v4().simple()));
    ...
    async fn get_macos_version(&self) -> PlatformResult<String> {
        let output = Command::new("sw_vers")
            ...
            .output()
            .map_err(|e| {
                PlatformError::DeviceInfo(format!("Failed to get macOS version: {}", e))
            })?;
        ...
        let version = String::from_utf8(output.stdout)
            .map_err(|e| {
                PlatformError::DeviceInfo(format!("Invalid UTF-8 in version output: {}", e))
            })?
            .trim()
            .to_string();
    ...
    async fn get_hardware_model(&self) -> PlatformResult<String> {
        let output = Command::new("sysctl")
            ...
            .output()
            .map_err(|e| {
                PlatformError::DeviceInfo(format!("Failed to get hardware model: {}", e))
            })?;
        ...
        let model = String::from_utf8(output.stdout)
            .map_err(|e| {
                PlatformError::DeviceInfo(format!("Invalid UTF-8 in model output: {}", e))
            })?
            .trim()
            .to_string();
    ...
    async fn get_total_memory(&self) -> PlatformResult<u64> {
        ...
        let memory_str = String::from_utf8(output.stdout).map_err(|e| {
            PlatformError::DeviceInfo(format!("Invalid UTF-8 in memory output: {}", e))
        })?;

        let total_memory = memory_str.trim().parse::<u64>().map_err(|e| {
            PlatformError::DeviceInfo(format!("Failed to parse memory size: {}", e))
        })?;
    ...
    async fn get_cpu_frequency(&self) -> PlatformResult<u32> {
        let output = Command::new("sysctl")
            ...
            .output()
            .map_err(|e| {
                PlatformError::DeviceInfo(format!("Failed to get CPU frequency: {}", e))
            })?;
        ...
        let freq_str = String::from_utf8(output.stdout).map_err(|e| {
            PlatformError::DeviceInfo(format!("Invalid UTF-8 in CPU frequency: {}", e))
        })?;
        ...
        let freq_hz = freq_str.parse::<u64>().map_err(|e| {
            PlatformError::DeviceInfo(format!("Failed to parse CPU frequency: {}", e))
        })?;
...
mod tests {
    ...
    async fn test_uptime_info() {
        ...
        let uptime_info = monitor
            .get_uptime()
            .await
            .expect("Failed to get uptime info");
    ...
    async fn test_parse_vm_stat_line() {
        ...
        let result =
            monitor.parse_vm_stat_line("Pages free:                               123456.");
        assert_eq!(result, Some(("free".to_string(), 123456)));

        let result =
            monitor.parse_vm_stat_line("Pages inactive:                           789012.");
