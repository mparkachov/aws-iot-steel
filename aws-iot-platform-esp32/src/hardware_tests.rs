...
impl ESP32HardwareTests {
    ...
    async fn test_sleep_accuracy(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for duration in test_durations {
            ...
            info!("Sleep accuracy for {:?}: {:.2}% (actual: {:?})", duration, accuracy, elapsed);
            
            // Allow 10% tolerance for timing accuracy
            assert!(
                elapsed >= duration && elapsed < duration + Duration::from_millis(duration.as_millis() as u64 / 10),
    ...
    async fn test_memory_monitoring(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        info!("Baseline memory: {} bytes free, {} bytes used", 
        ...
        for i in 0..10 {
            ...
            info!("After allocation {}: {} bytes free, {} bytes used", 
                  i + 1, current_memory.free_bytes, current_memory.used_bytes);
        ...
        info!("Final memory: {} bytes free, {} bytes used", 
    ...
    async fn test_secure_storage_persistence(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(loaded_data.as_ref(), Some(test_data), "Data should be immediately readable");
        ...
        for i in 0..5 {
            ...
            let loaded_data = self.hal.load_secure_data(test_key).await?;
            assert_eq!(loaded_data.as_ref(), Some(test_data), 
                      "Data should persist through operations (iteration {})", i + 1);
        }
    ...
    async fn test_power_management(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        info!("Initial state - Memory: {} bytes free, Uptime: {:?}", 
        ...
        for (i, duration) in sleep_durations.iter().enumerate() {
            info!("Power management test cycle {} - sleeping for {:?}", i + 1, duration);
            ...
            info!("Post-sleep state - Memory: {} bytes free, Uptime: {:?}, Sleep accuracy: {:?}", 
                  post_sleep_memory.free_bytes, post_sleep_uptime.uptime, sleep_elapsed);
            
            // Verify system is still responsive
            assert!(post_sleep_uptime.uptime > initial_uptime.uptime, 
    ...
    async fn test_gpio_stability(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for i in 0..100 {
            ...
            assert_eq!(read_state, state, "LED state should be consistent (iteration {})", i + 1);
    ...
    pub async fn test_device_info_consistency(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for i in 0..5 {
            ...
            info!("Device info reading {}: ID={}, Platform={}", 
                  i + 1, device_infos[i].device_id, device_infos[i].platform);
        ...
        for (i, info) in device_infos.iter().enumerate().skip(1) {
            assert_eq!(info.device_id, first_info.device_id, 
                      "Device ID should be consistent (reading {})", i + 1);
            assert_eq!(info.platform, first_info.platform, 
                      "Platform should be consistent (reading {})", i + 1);
            assert_eq!(info.firmware_version, first_info.firmware_version, 
                      "Firmware version should be consistent (reading {})", i + 1);
        }
...
mod tests {
    ...
    async fn test_hardware_validation() {
        ...
        assert!(result.is_ok(), "Hardware validation should succeed: {:?}", result);
    }
}