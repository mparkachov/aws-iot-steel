...
impl ESP32HAL {
    ...
    fn get_esp32_memory_info() -> PlatformResult<MemoryInfo> {
        unsafe {
            ...
            let largest_free = esp_idf_sys::heap_caps_get_largest_free_block(
    ...
    fn init_nvs(&self) -> PlatformResult<EspNvs<NvsDefault>> {
        ...
        let nvs_default_partition = EspNvsPartition::<NvsDefault>::take()
            .map_err(|e| PlatformError::Storage(format!("Failed to take NVS partition: {:?}", e)))?;
        
        // Open NVS namespace for secure storage
        let nvs = EspNvs::new(nvs_default_partition, SECURE_STORAGE_NAMESPACE, true)
            .map_err(|e| PlatformError::Storage(format!("Failed to open NVS namespace: {:?}", e)))?;
    ...
    fn validate_storage_key(key: &str) -> PlatformResult<()> {
        if key.is_empty() {
            return Err(PlatformError::Storage("Storage key cannot be empty".to_string()));
        }
        
        if key.len() > MAX_KEY_LENGTH {
            return Err(PlatformError::Storage(
                format!("Storage key too long: {} > {}", key.len(), MAX_KEY_LENGTH)
            ));
        }
...
impl PlatformHAL for ESP32HAL {
    ...
    async fn set_led(&self, state: LedState) -> PlatformResult<()> {
        ...
        let mut led_pin_guard = self.led_pin.lock()
        ...
        if let Some(ref mut led_pin) = *led_pin_guard {
            match state {
                LedState::On => {
                    led_pin.set_high()
                        .map_err(|e| PlatformError::Led(format!("Failed to turn LED on: {:?}", e)))?;
                    log::info!("ESP32-C3 LED turned ON");
                },
                LedState::Off => {
                    led_pin.set_low()
                        .map_err(|e| PlatformError::Led(format!("Failed to turn LED off: {:?}", e)))?;
            ...
            *led_state_guard = state;
            ...
            Err(PlatformError::Hardware("LED pin not initialized".to_string()))
        }
    }
    ...
    async fn store_secure_data(&self, key: &str, data: &[u8]) -> PlatformResult<()> {
        ...
        if data.len() > MAX_DATA_SIZE {
            return Err(PlatformError::Storage(
                format!("Data too large: {} > {}", data.len(), MAX_DATA_SIZE)
            ));
        }
        
        log::info!("Storing secure data for key '{}' ({} bytes)", key, data.len());
        
        let nvs_guard = self.nvs.lock()
            ...
            match nvs.blob_len(key) {
                Ok(Some(size)) => {
                    ...
                    match nvs.get_blob(key, &mut buffer) {
                        ...
                        Err(e) => Err(PlatformError::Storage(
                ...
                Err(e) => Err(PlatformError::Storage(
                    format!("Failed to get data size: {:?}", e)
                )),
            }
        } else {
            Err(PlatformError::Storage("NVS not initialized".to_string()))
    ...
    async fn initialize(&mut self) -> PlatformResult<()> {
        ...
        if *initialized_guard {
            return Err(PlatformError::Hardware("ESP32-C3 HAL already initialized".to_string()));
        }
        ...
        let peripherals = Peripherals::take()
            .map_err(|_| PlatformError::Hardware("Failed to take ESP32-C3 peripherals".to_string()))?;
        
        // Initialize LED GPIO pin
        log::info!("Initializing LED on GPIO{}", LED_GPIO_PIN);
        let led_pin = PinDriver::output(peripherals.pins.gpio2)
            .map_err(|e| PlatformError::Hardware(format!("Failed to initialize LED pin: {:?}", e)))?;
        ...
        {
            let mut nvs_guard = self.nvs.lock()
                .map_err(|_| PlatformError::Storage("Failed to lock NVS".to_string()))?;
            *nvs_guard = Some(nvs);
        }
    ...
    async fn shutdown(&mut self) -> PlatformResult<()> {
        ...
        if !*initialized_guard {
            return Err(PlatformError::Hardware("ESP32-C3 HAL not initialized".to_string()));
        }
