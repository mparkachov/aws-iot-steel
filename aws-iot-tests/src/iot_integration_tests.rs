use aws_iot_core::{DeviceState, IoTClientTrait, ProgramMessage};
use std::sync::Arc;
...
impl IoTTestConfig {
    ...
    pub fn has_real_credentials(&self) -> bool {
        self.cert_path.is_some() && self.private_key_path.is_some() && !self.use_mock
    }
}
...
impl IoTIntegrationTests {
    ...
    pub async fn run_all_tests(&self) -> Result<TestResults, Box<dyn std::error::Error>> {
        ...
        results.add_test_result("iot_connectivity", self.test_iot_connectivity().await);

        // Test program delivery
        results.add_test_result("program_delivery", self.test_program_delivery().await);
    ...
    async fn run_mock_tests(&self) -> Result<TestResults, Box<dyn std::error::Error>> {
        ...
        let mut results = TestResults::new();
        let client = Arc::new(tokio::sync::Mutex::new(MockIoTClient::new(
            self.config.thing_name.clone(),
        )));
    ...
    async fn test_iot_connectivity(&self) -> TestResult {
        ...
        match timeout(test_timeout, self.connect_to_iot()).await {
            Ok(Ok(client)) => {
                ...
                match client
                    .publish("test/connectivity", b"ping", rumqttc::QoS::AtLeastOnce)
                    .await
                {
    ...
    async fn test_resilience_impl(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        client
            .publish(
                "test/resilience/recovered",
                b"connection recovered",
                rumqttc::QoS::AtLeastOnce,
            )
            .await?;
    ...
    async fn test_concurrent_execution_impl(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let programs = [
            ...
            (
                "logging-test",
                r#"(begin (log-info "Concurrent test") (log-debug "Debug message"))"#,
            ),
        ];
    ...
    async fn test_mock_connectivity(
        &self,
        client: Arc<tokio::sync::Mutex<crate::MockIoTClient>>,
    ) -> TestResult {
        ...
        match client_guard.connect().await {
            Ok(_) => {
                match client_guard
                    .publish("test/mock", b"test message", rumqttc::QoS::AtLeastOnce)
                    .await
                {
    ...
    /// Test mock program delivery
    async fn test_mock_program_delivery(
        &self,
        client: Arc<tokio::sync::Mutex<crate::MockIoTClient>>,
    ) -> TestResult {
        let mut client_guard = client.lock().await;
    ...
    /// Test mock shadow operations
    async fn test_mock_shadow_operations(
        &self,
        client: Arc<tokio::sync::Mutex<crate::MockIoTClient>>,
    ) -> TestResult {
        let mut client_guard = client.lock().await;
    ...
    async fn connect_to_iot(
        &self,
    ) -> Result<Box<dyn aws_iot_core::IoTClientTrait>, Box<dyn std::error::Error>> {
    ...
    async fn test_iot_integration_suite() {
        ...
        if std::env::var("CI").is_ok() {
            assert_eq!(
                ...
                0,
                "Some IoT integration tests failed"
            );
        }
    }
