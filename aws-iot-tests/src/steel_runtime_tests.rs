...
mod tests {
    ...
    use aws_iot_core::steel_runtime::{SteelError, ProgramHandle};
    ...
    async fn test_program_loading() {
        ...
        let result = runtime.load_program(program_code, Some("test-program")).await;
        assert!(result.is_ok());
    ...
    async fn test_program_execution() {
        ...
        let handle = runtime.load_program(program_code, Some("calc-program")).await
    ...
    async fn test_program_execution_failure() {
        ...
        let handle = runtime.load_program(program_code, None).await
            .expect("Failed to load program");
    ...
    async fn test_program_management() {
        ...
        let program1 = runtime.load_program("(+ 1 2)", Some("prog1")).await
            .expect("Failed to load program 1");
        let _program2 = runtime.load_program("(* 3 4)", Some("prog2")).await
    ...
    async fn test_event_handling() {
        ...
        let handler_program = runtime.load_program("(display \"Event handled\")", Some("handler")).await
            .expect("Failed to load handler program");
        
        // Register event handler
        let result = runtime.register_event_handler("test-event", handler_program).await;
        assert!(result.is_ok());
        ...
        let result = runtime.emit_event("unknown-event", SteelValue::BoolV(true)).await;
        assert!(result.is_err());
    }
    ...
    async fn test_runtime_cleanup() {
        ...
        let _handle1 = runtime.load_program("(+ 1 2)", Some("prog1")).await.unwrap();
        let _handle2 = runtime.load_program("(* 3 4)", Some("prog2")).await.unwrap();
        runtime.execute_code("(+ 5 6)").await.unwrap();
        runtime.set_global_variable("test", SteelValue::NumV(42.0)).await.unwrap();
