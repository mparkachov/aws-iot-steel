...
use aws_iot_core::steel_runtime::{
    ExecutionContext, ExecutionStats, ProgramHandle, ProgramInfo, SteelError, SteelResult,
};
...
impl SteelRuntime for MockSteelRuntime {
    async fn load_program(
        ...
        self.programs
            .lock()
            .await
            .insert(handle.clone(), stored_program);
        Ok(handle)
    }
