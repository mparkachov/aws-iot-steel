...
mod tests {
    ...
    async fn test_logging_api() {
        ...
        api.log(aws_iot_core::LogLevel::Info, "Test info message")
            .unwrap();
        api.log(aws_iot_core::LogLevel::Warn, "Test warning message")
            .unwrap();
        api.log(aws_iot_core::LogLevel::Error, "Test error message")
            .unwrap();
        api.log(aws_iot_core::LogLevel::Debug, "Test debug message")
            .unwrap();
        ...
        api.log(aws_iot_core::LogLevel::Info, &long_message)
            .unwrap();
    }
    ...
    async fn test_mqtt_publish_api() {
        ...
        let result = api
            .publish_mqtt("test/topic/with/+/wildcard", "message")
            .await;
        assert!(result.is_err());
    }
    ...
    async fn test_shadow_update_api() {
        ...
        // Test null value
        let result = api
            .update_shadow("null_test", serde_json::Value::Null)
            .await;
        assert!(result.is_ok());
    }
    ...
    async fn test_concurrent_api_calls() {
        ...
        for _i in 0..10 {
            ...
            let handle = tokio::spawn(async move { api_clone.sleep(0.1).await });
            handles.push(handle);
        }
        ...
        for i in 0..10 {
            let api_clone = api.clone();
            let state = i % 2 == 0;
            let handle = tokio::spawn(async move { api_clone.set_led(state).await });
            handles.push(handle);
        }
    ...
    async fn test_api_state_consistency() {
        ...
        let loaded = api
            .load_data(key)
            .await
            .expect("Failed to load deleted data");
