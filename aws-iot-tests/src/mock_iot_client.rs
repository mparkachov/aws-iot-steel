...
use aws_iot_core::{
    ConnectionStatus, DeviceState, IoTClientTrait, IoTError, IoTResult, ProgramMessage,
};
...
impl MockIoTClient {
    ...
    pub async fn publish_string(&self, topic: &str, payload: &str) -> IoTResult<()> {
        self.publish(topic, payload.as_bytes(), QoS::AtMostOnce)
            .await
    }
    ...
    pub async fn request_program(&self, program_name: &str) -> IoTResult<()> {
        ...
        self.publish_string(&request_topic, &request_payload.to_string())
            .await
    }
