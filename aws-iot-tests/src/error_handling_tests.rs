...
mod tests {
    use aws_iot_core::{
        IoTError, LedState, PlatformHAL, IoTClientTrait, SteelRuntime
    };
    ...
    async fn test_iot_client_error_scenarios() {
        ...
        let result = client.publish("test/topic", b"message", rumqttc::QoS::AtMostOnce).await;
        assert!(matches!(result, Err(IoTError::NotConnected)));
        
        let result = client.subscribe("test/topic", rumqttc::QoS::AtMostOnce).await;
        assert!(matches!(result, Err(IoTError::NotConnected)));
        ...
        assert!(client.publish("test/topic", b"message", rumqttc::QoS::AtMostOnce).await.is_ok());
        assert!(client.subscribe("test/topic", rumqttc::QoS::AtMostOnce).await.is_ok());
    }
    ...
    async fn test_api_parameter_validation() {
        ...
        for topic in invalid_topics {
            ...
            let is_valid = !topic.is_empty() && 
                          !topic.contains('+') && 
                          !topic.contains('#');
            assert!(!is_valid || topic.is_empty(), 
    ...
    async fn test_concurrent_error_scenarios() {
        let client = std::sync::Arc::new(tokio::sync::Mutex::new(
            MockIoTClient::new("test-device".to_string())
        ));
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                ...
                client.publish(&format!("test/topic/{}", i), b"message", rumqttc::QoS::AtMostOnce).await
            });
            handles.push(handle);
        }
    ...
    async fn test_resource_exhaustion_scenarios() {
        ...
        for handle in handles.into_iter().take(10) { // Test first 10 to avoid too much overhead
            let result = runtime.execute_program(handle).await;
            assert!(result.is_ok());
        }
        ...
        for i in 0..100 {
            let result = runtime.set_global_variable(
                &format!("var_{}", i), 
    ...
    async fn test_malformed_data_handling() {
        ...
        let result = client.publish("test/topic", malformed_json.as_bytes(), rumqttc::QoS::AtMostOnce).await;
        ...
        let result = client.publish("test/topic", large_payload.as_bytes(), rumqttc::QoS::AtMostOnce).await;
        assert!(result.is_ok());
        
        // Test binary data in string context
        let binary_data = String::from_utf8_lossy(&[0, 1, 2, 255, 254, 253]);
        let result = client.publish("test/topic", binary_data.as_bytes(), rumqttc::QoS::AtMostOnce).await;
        assert!(result.is_ok());
    }
    ...
    async fn test_state_corruption_recovery() {
        ...
        for i in 0..100 {
            let new_state = if i % 2 == 0 { LedState::On } else { LedState::Off };
    ...
    async fn test_error_propagation() {
        ...
        let publish_result = client.publish("test/topic", b"message", rumqttc::QoS::AtMostOnce).await;
