...
use aws_iot_core::{
    DeviceInfo, EmbeddedSteelRuntime, LedState, MemoryInfo, MemoryUsageStats, PlatformHAL,
    PlatformResult, UptimeInfo,
};
use chrono::Utc;
...
impl BenchmarkResults {
    fn print_summary(&self) {
        ...
        println!(
            "  Program Size: {} bytes",
            self.memory_stats.total_program_size
        );
    }
}
...
impl EmbeddedSteelBenchmark {
    ...
    async fn benchmark_led_operations(
        &mut self,
        iterations: u32,
    ) -> Result<BenchmarkResults, Box<dyn std::error::Error>> {
        info!(
            "Running LED operations benchmark with {} iterations",
            iterations
        );
        ...
        for i in 0..iterations {
            ...
            match self
                .runtime
                .load_program(program_code, Some(&program_name))
                .await
            {
                Ok(handle) => {
    ...
    async fn benchmark_sleep_operations(
        &mut self,
        iterations: u32,
    ) -> Result<BenchmarkResults, Box<dyn std::error::Error>> {
        info!(
            "Running sleep operations benchmark with {} iterations",
            iterations
        );
    ...
    async fn benchmark_blink_operations(
        &mut self,
        iterations: u32,
    ) -> Result<BenchmarkResults, Box<dyn std::error::Error>> {
        info!(
            "Running blink operations benchmark with {} iterations",
            iterations
        );
    ...
    async fn benchmark_system_info(
        &mut self,
        iterations: u32,
    ) -> Result<BenchmarkResults, Box<dyn std::error::Error>> {
        info!(
            "Running system info benchmark with {} iterations",
            iterations
        );
    ...
    async fn benchmark_memory_limits(
        &mut self,
    ) -> Result<BenchmarkResults, Box<dyn std::error::Error>> {
        ...
        loop {
            let program_code = "(begin (led-on) (sleep 0.001) (led-off))".to_string();
                ...
                .await
            {
                Ok(handle) => {
                    ...
                    if programs_loaded >= 50 {
                        // Safety limit for benchmark
                        break;
                    }
                }
                Err(_) => {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let max_memory_usage = results
        .iter()
        .map(|r| r.memory_stats.heap_peak_bytes)
        .max()
        .unwrap_or(0);
    ...
    #[cfg(target_arch = "riscv32")]
    {
        ...
        if max_memory_usage <= EMBEDDED_MEMORY_LIMIT {
            println!(
                "✅ Memory Usage: WITHIN LIMITS ({} / {} bytes)",
                max_memory_usage, EMBEDDED_MEMORY_LIMIT
            );
        } else {
            println!(
                "❌ Memory Usage: EXCEEDS LIMITS ({} / {} bytes)",
                max_memory_usage, EMBEDDED_MEMORY_LIMIT
            );
        }
    }
    ...
    {
        println!(
            "ℹ️  Memory Usage: {} bytes (desktop target)",
            max_memory_usage
        );
    }
    ...
    if total_operations > 100.0 {
        println!(
            "✅ Performance: EXCELLENT ({:.2} ops/sec)",
            total_operations
        );
