use aws_iot_core::{
    SteelTestRunner, PlatformHAL, PlatformResult, 
    LedState, DeviceInfo, MemoryInfo, UptimeInfo
};
    ...
    if let Some(test_file) = matches.get_one::<String>("file") {
        ...
        match runner.run_test_file(&path.to_string_lossy()).await {
            Ok(()) => {
                ...
                info!("✓ Test file passed: {} (completed in {:?})", test_file, duration);
            ...
            Err(e) => {
                let duration = start_time.elapsed();
                error!("✗ Test file failed: {} - {} (failed after {:?})", test_file, e, duration);
        ...
        match runner.run_test_directory(test_dir).await {
            ...
            Err(e) => {
                let total_duration = start_time.elapsed();
                error!("Failed to run test directory: {} (failed after {:?})", e, total_duration);
                std::process::exit(1);
            }
        }
    }
...
fn print_detailed_results(results: &TestResults, total_duration: std::time::Duration, verbose: bool) {
    info!("");
    ...
    if verbose {
        ...
        if results.total() > 0 {
            ...
            info!("  Tests per second: {:.2}", results.total() as f64 / total_duration.as_secs_f64());
        }
    }
