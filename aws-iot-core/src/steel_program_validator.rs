...
impl SteelProgramValidator {
            ...
            match ch {
                ...
                '(' if !in_string => {
                    ...
                    if !current_token.is_empty() {
                        self.process_token(&current_token, &mut variables, &mut functions, &mut in_define, result);
                        current_token.clear();
                    }
                }
                ')' if !in_string => {
    ...
    fn process_token(
        ...
        if self.allowed_functions.contains(token) {
            functions.insert(token.to_string());
                  ...
                  !token.starts_with('#') { // Not a special form
    ...
    fn is_potentially_unsafe_function(&self, token: &str) -> bool {
        let unsafe_functions = [
            "eval", "apply", "call/cc", "call-with-current-continuation",
            "load", "read", "write", "display", "newline",
            "file-exists?", "delete-file", "open-input-file", "open-output-file",
            "system", "exit", "abort", "error",
        ];
    ...
    fn optimize_patterns(&self, program: &str) -> String {
        ...
        // Replace (if condition #t #f) with condition
        optimized = optimized.replace("(if ", "(if ");
