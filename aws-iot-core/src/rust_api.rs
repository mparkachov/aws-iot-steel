...
use crate::{
    DeviceInfo, LedState, LogLevel, MemoryInfo, PlatformError, PlatformHAL, SystemError,
    SystemResult,
};
...
impl TimerManager {
            ...
            loop {
                ...
                // Update task info
                {
                    ...
                    if let Some(task) = tasks.get_mut(&task_handle) {
                        ...
                        task.run_count += 1;
                        task.next_run =
                            Some(Utc::now() + chrono::Duration::from_std(interval).unwrap());
                    }
                }
    ...
    fn parse_simple_cron(&self, cron_expr: &str) -> SystemResult<Duration> {
        ...
        if parts[0].starts_with("*/") {
            // Every N seconds
            let seconds: u64 = parts[0][2..].parse().map_err(|_| {
                SystemError::Configuration("Invalid seconds in cron expression".to_string())
            })?;
        ...
        if parts[1].starts_with("*/") && parts[0] == "0" {
            // Every N minutes
            let minutes: u64 = parts[1][2..].parse().map_err(|_| {
                SystemError::Configuration("Invalid minutes in cron expression".to_string())
            })?;
        ...
        if parts[2].starts_with("*/") && parts[0] == "0" && parts[1] == "0" {
            // Every N hours
            let hours: u64 = parts[2][2..].parse().map_err(|_| {
                SystemError::Configuration("Invalid hours in cron expression".to_string())
            })?;
        ...
        Err(SystemError::Configuration(
            "Unsupported cron expression. Use patterns like '*/30 * * * *' for every 30 seconds"
                .to_string(),
        ))
    }
}
...
impl RustAPI {
    ...
    pub async fn get_sensor_data(&self) -> SystemResult<SensorData> {
        ...
        let temperature =
            self.generate_sensor_value(config.temperature_range.0, config.temperature_range.1, 0.5);

        let humidity =
            self.generate_sensor_value(config.humidity_range.0, config.humidity_range.1, 2.0);

        let pressure =
            self.generate_sensor_value(config.pressure_range.0, config.pressure_range.1, 1.0);
    ...
    pub async fn store_data(&self, key: &str, value: &str) -> SystemResult<()> {
        ...
        self.hal
            .store_secure_data(key, value.as_bytes())
            .await
        ...
        match self.hal.load_secure_data(key).await? {
            Some(data) => {
                let value = String::from_utf8(data).map_err(|e| {
                    SystemError::Platform(PlatformError::Storage(format!(
                        "Invalid UTF-8 data: {}",
                        e
                    )))
                })?;
    ...
    pub async fn list_storage_keys(&self) -> SystemResult<Vec<String>> {
        self.hal.list_secure_keys().await.map_err(SystemError::from)
    }
    ...
    pub async fn get_device_info(&self) -> SystemResult<DeviceInfo> {
        self.hal.get_device_info().await.map_err(SystemError::from)
    }
    ...
    pub async fn get_memory_info(&self) -> SystemResult<MemoryInfo> {
        self.hal.get_memory_info().await.map_err(SystemError::from)
    }
    ...
    pub fn update_device_config(&self, config: DeviceConfig) -> SystemResult<()> {
        ...
        if config.sensor_update_interval == 0 {
            return Err(SystemError::Configuration(
                "Sensor update interval must be positive".to_string(),
            ));
        }
        ...
        if config.max_storage_keys == 0 {
            return Err(SystemError::Configuration(
                "Max storage keys must be positive".to_string(),
            ));
        }
    ...
    pub async fn save_device_config(&self) -> SystemResult<()> {
        ...
        let config_json = serde_json::to_string(&config).map_err(SystemError::Serialization)?;
        ...
        match self.load_data("device_config").await? {
            Some(config_json) => {
                let config: DeviceConfig =
                    serde_json::from_str(&config_json).map_err(SystemError::Serialization)?;
    ...
    pub fn set_timer(
        ...
        if duration_secs > 86400.0 {
            return Err(SystemError::Configuration(
                "Timer duration cannot exceed 24 hours".to_string(),
            ));
        }
        ...
        if callback_code.trim().is_empty() {
            return Err(SystemError::Configuration(
                "Callback code cannot be empty".to_string(),
            ));
        }
        ...
        self.timer_manager
            .set_timer(name.to_string(), duration, callback)
    }
    ...
    pub fn schedule_task(
        ...
        self.timer_manager
            .schedule_task(name.to_string(), cron_expr.to_string(), callback)
    }
    ...
    fn generate_light_level(&self, config: &SensorConfig, now: DateTime<Utc>) -> f64 {
        ...
        // Add some variation
        let variation =
            self.generate_sensor_value(-config.light_variation, config.light_variation, 1.0);
    ...
    pub async fn publish_mqtt(&self, topic: &str, message: &str) -> Result<(), crate::APIError> {
        ...
        if topic.contains('+') || topic.contains('#') {
            return Err(APIError::InvalidParameter(
                "Topic cannot contain wildcards".to_string(),
            ));
        }
    ...
    pub async fn update_shadow(
        ...
        if key.is_empty() {
            return Err(APIError::InvalidParameter(
                "Shadow key cannot be empty".to_string(),
            ));
        }
...
mod tests {
    use super::*;
    use crate::{DeviceInfo, MemoryInfo, PlatformError, PlatformResult, UptimeInfo};
    ...
    async fn test_timer_management() {
        ...
        let timer_handle = api
            .set_timer("test_timer", 0.1, "(log-info \"Timer fired!\")")
            .unwrap();
    ...
    async fn test_task_scheduling() {
        ...
        let task_handle = api
            .schedule_task("test_task", "*/5 * * * *", "(log-info \"Task executed!\")")
            .unwrap();
    ...
    async fn test_timer_firing() {
        ...
        let _timer_handle = api
            .set_timer("quick_timer", 0.01, "(log-info \"Quick timer!\")")
            .unwrap();
