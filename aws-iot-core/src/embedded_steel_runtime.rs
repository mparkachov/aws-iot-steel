...
use std::collections::HashMap as FnvIndexMap;
        ...
        #[cfg(target_arch = "riscv32")]
        {
            ...
            if stats.heap_used_bytes > MAX_HEAP_USAGE {
                return Err(SystemError::Configuration(
                    format!("Heap usage exceeded limit: {} > {}", stats.heap_used_bytes, MAX_HEAP_USAGE)
                ));
            }
            
            if stats.stack_used_bytes > MAX_STACK_USAGE {
                return Err(SystemError::Configuration(
                    format!("Stack usage exceeded limit: {} > {}", stats.stack_used_bytes, MAX_STACK_USAGE)
                ));
            }
        }
...
impl EmbeddedSteelProgram {
    pub fn new(
        ...
        if code.len() > MAX_PROGRAM_SIZE {
            return Err(SystemError::Configuration(
                format!("Program too large: {} > {}", code.len(), MAX_PROGRAM_SIZE)
            ));
        }
        
        let mut code_string = HeaplessString::new();
        code_string.push_str(code).map_err(|_| {
            SystemError::Configuration("Failed to store program code".to_string())
        })?;
...
impl EmbeddedProgramStorage {
    ...
    pub fn load_program(
        ...
        if self.programs.len() >= MAX_PROGRAMS {
            return Err(SystemError::Configuration(
                format!("Too many programs loaded: {} >= {}", self.programs.len(), MAX_PROGRAMS)
            ));
        }
        ...
        info!("Loaded embedded Steel program: handle={}, size={} bytes", handle.id(), program_size);
        Ok(handle)
    }
    ...
    pub fn get_program_mut(&mut self, handle: &EmbeddedProgramHandle) -> Option<&mut EmbeddedSteelProgram> {
    ...
    pub fn update_execution_stats(&mut self, handle: &EmbeddedProgramHandle, execution_time: Duration) -> SystemResult<()> {
...
impl EmbeddedSteelRuntimeAPI {
    ...
    pub async fn memory_info(&self) -> SystemResult<SteelVal> {
        ...
        info.push(SteelVal::StringV(format!("hal-total: {}", hal_memory.total_bytes).into()));
        info.push(SteelVal::StringV(format!("hal-free: {}", hal_memory.free_bytes).into()));
        info.push(SteelVal::StringV(format!("hal-used: {}", hal_memory.used_bytes).into()));
        info.push(SteelVal::StringV(format!("runtime-heap: {}", runtime_stats.heap_used_bytes).into()));
        info.push(SteelVal::StringV(format!("runtime-peak: {}", runtime_stats.heap_peak_bytes).into()));
        info.push(SteelVal::StringV(format!("programs: {}", runtime_stats.programs_loaded).into()));
        info.push(SteelVal::StringV(format!("program-size: {}", runtime_stats.total_program_size).into()));
    ...
    pub async fn device_info(&self) -> SystemResult<SteelVal> {
        ...
        match self.hal.get_device_info().await {
            Ok(info) => {
                ...
                pairs.push(SteelVal::StringV(format!("firmware: {}", info.firmware_version).into()));
...
impl EmbeddedSteelRuntime {
    ...
    pub fn new(hal: Arc<dyn PlatformHAL>) -> SystemResult<Self> {
        ...
        let rust_api = Arc::new(EmbeddedSteelRuntimeAPI::new(hal, Arc::clone(&memory_monitor)));
        let program_storage = Arc::new(Mutex::new(EmbeddedProgramStorage::new(Arc::clone(&memory_monitor))));
    ...
    fn register_embedded_functions(engine: &mut Engine) -> SystemResult<()> {
        ...
        match engine.compile_and_run_raw_program(prelude) {
            ...
            Err(e) => {
                ...
                Err(SystemError::Configuration(format!("Embedded Steel prelude failed: {:?}", e)))
            }
        }
    }
    ...
    pub async fn load_program(
        ...
        if let Some(name) = name {
            metadata.name.push_str(&name[..name.len().min(31)]).map_err(|_| {
                SystemError::Configuration("Program name too long".to_string())
            })?;
        }
        metadata.version.push_str("1.0.0").map_err(|_| {
            SystemError::Configuration("Failed to set version".to_string())
        })?;
    ...
    pub async fn execute_program(
        ...
        match result {
            Ok(Ok(value)) => {
                info!("Embedded Steel program completed: handle={}, time={:?}", handle.id(), execution_time);
                Ok(value)
            }
            Ok(Err(e)) => {
                error!("Embedded Steel program failed: handle={}, error={}", handle.id(), e);
                Err(e)
            }
            Err(_) => {
                error!("Embedded Steel program timeout: handle={}, time={:?}", handle.id(), execution_time);
    ...
    async fn handle_blink_led(&self, code: &str) -> SystemResult<SteelVal> {
        ...
        if let Some(start) = code.find("(blink-led ") {
            ...
            if let Some(end) = after_blink.find(')') {
                ...
                if parts.len() == 2 {
                    if let (Ok(times), Ok(delay)) = (parts[0].parse::<u32>(), parts[1].parse::<f64>()) {
                        // Limit blink times for embedded systems
                        let times = times.min(20); // Max 20 blinks
                        let delay = delay.max(0.1).min(5.0); // 0.1s to 5s delay
        ...
        Err(SystemError::Configuration("Invalid blink-led syntax".to_string()))
    }
...
mod tests {
    ...
    async fn test_embedded_program_execution() {
        ...
        let handle = runtime.load_program("(led-on)", Some("test")).await.unwrap();
    ...
    async fn test_blink_led_optimization() {
        ...
        let handle = runtime.load_program("(blink-led 3 0.1)", Some("blink")).await.unwrap();
        let result = runtime.execute_program(handle).await;
        assert!(result.is_ok());
    }
