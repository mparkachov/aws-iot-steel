...
async fn test_multiple_programs() {
    ...
    for i in 1..=5 {
        ...
        manager.handle_program_message(&program_message).await.unwrap();
    }
...
async fn test_program_delivery_manager_with_real_iot_client() {
    ...
    let result = manager.execute_program("integration-test-program").await.unwrap();
    assert!(result.success);
...
async fn test_checksum_validation() {
    ...
    let manager = ProgramDeliveryManager::new(
        "checksum-test-device".to_string(),
        Arc::new(iot_client),
    );
    ...
    assert!(!validation.checksum_match);
    assert!(validation.error_message.is_some());
    assert!(validation.error_message.unwrap().contains("Checksum mismatch"));
}
...
async fn test_program_metadata_handling() {
    ...
    assert_eq!(metadata.description, Some("Comprehensive sensor reading program with metadata".to_string()));
...
async fn test_concurrent_program_operations() {
    let manager = Arc::new(MockProgramDeliveryManager::new("concurrent-test-device".to_string()));
    ...
    for i in 1..=10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            manager_clone.execute_program(&format!("concurrent-program-{:02}", i)).await.unwrap()
        });
        handles.push(handle);
    }
