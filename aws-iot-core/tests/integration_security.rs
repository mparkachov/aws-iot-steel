...
async fn test_certificate_expiration_validation() {
    ...
    store.store_certificate(TEST_CERT_PEM, "test_cert").await.unwrap();
    
    // Test expiration validation
    let is_valid = store.validate_certificate_expiration("test_cert").await.unwrap();
...
async fn test_certificate_removal() {
    ...
    store.store_certificate(TEST_CERT_PEM, "test_cert").await.unwrap();
    store.store_private_key(TEST_KEY_PEM, "test_cert").await.unwrap();
...
async fn test_security_manager_certificate_operations() {
    ...
    let cert_info = manager.store_certificate_with_key(
        TEST_CERT_PEM,
        TEST_KEY_PEM,
        "test_cert"
    ).await.unwrap();
...
async fn test_security_manager_cryptographic_operations() {
    ...
    let is_valid = manager.verify_signature(&public_key, message, &signature).unwrap();
    ...
    let is_valid_empty = manager.verify_signature(&public_key, message, &empty_signature).unwrap();
...
async fn test_multiple_certificates() {
    ...
    manager.store_certificate_with_key(TEST_CERT_PEM, TEST_KEY_PEM, "cert_1").await.unwrap();
    manager.store_certificate_with_key(TEST_CERT_PEM, TEST_KEY_PEM, "cert_2").await.unwrap();
    manager.store_certificate_with_key(TEST_CERT_PEM, TEST_KEY_PEM, "cert_3").await.unwrap();
...
async fn test_program_signature_verification() {
    ...
    let is_valid = manager.verify_program_signature(program_data, &signature, public_key_id).unwrap();
    assert!(is_valid);
    
    // Test empty signature
    let empty_signature = vec![];
    let is_valid = manager.verify_program_signature(program_data, &empty_signature, public_key_id).unwrap();
    assert!(!is_valid);
    
    // Test empty key ID
    let is_valid = manager.verify_program_signature(program_data, &signature, "").unwrap();
    assert!(!is_valid);
    
    // Test short signature
    let short_signature = vec![1u8; 32];
    let is_valid = manager.verify_program_signature(program_data, &short_signature, public_key_id).unwrap();
    assert!(!is_valid);
}
