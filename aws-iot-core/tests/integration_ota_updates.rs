use aws_iot_core::{
    DownloadProgress, FirmwareUpdateRequest, FirmwareUpdateStatus, InMemoryCertificateStore,
    IoTClientTrait, MockIoTClient, OTAManager, OTAManagerTrait, PreSignedUrlRequest,
    SecurityManager,
};
use chrono::Utc;
use std::sync::Arc;
...
async fn test_firmware_update_request() {
    ...
    assert!(matches!(
        status.status,
        FirmwareUpdateStatus::Requested | FirmwareUpdateStatus::Validating
    ));
}
...
async fn test_firmware_request_validation() {
    ...
    let result = ota_manager
        .validate_firmware_request(&valid_request)
        .await
        .unwrap();
    ...
    let result = ota_manager
        .validate_firmware_request(&invalid_request)
        .await
        .unwrap();
    assert!(!result.is_valid);
    assert!(!result.compatibility_verified);
...
async fn test_pre_signed_url_request() {
    ...
    let found_request = messages
        .iter()
        .any(|(topic, _)| topic == "downloads/test-device/firmware-request");
...
async fn test_firmware_download_validation() {
    ...
    let result = ota_manager
        .download_firmware("http://example.com/firmware.bin", "checksum", 1024)
        .await;
    ...
    let result = ota_manager
        .download_firmware("not-a-url", "checksum", 1024)
        .await;
    assert!(result.is_err());
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("Invalid download URL"));
}
...
async fn test_firmware_installation() {
    ...
    ota_manager
        ...
        .write()
        .await
        .insert(request.request_id.clone(), update_result);

    let result = ota_manager.install_firmware(firmware_data, &request).await;
    assert!(result.is_ok());

    // Check that status was updated
    let status = ota_manager
        .get_update_status(&request.request_id)
        .await
        .unwrap();
    assert!(status.is_some());
    assert!(matches!(
...
async fn test_update_cancellation() {
    ...
    ota_manager
        ...
        .write()
        .await
        .insert(request_id.to_string(), update_result);
...
async fn test_update_cancellation_invalid_states() {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("Cannot cancel completed update"));
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("Cannot cancel update during installation"));
}
...
async fn test_firmware_rollback() {
    ...
    ota_manager
        ...
        .write()
        .await
        .insert(request_id.to_string(), b"previous_firmware_data".to_vec());

    // Perform rollback
    let result = ota_manager
        .rollback_firmware(request_id, "Test rollback")
        .await;
    assert!(result.is_ok());
...
async fn test_firmware_rollback_no_data() {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("No rollback data available"));
}
